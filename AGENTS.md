...
### `trace.py`: 日志追踪模块
包装 `nfctester.trace`，提供 6 层日志系统：

**输出日志：**
*   `trace.app(msg)` / `trace.debug(msg)` / `trace.warning(msg)` / `trace.error(msg)`: 简洁写法，直接输出到对应层。
...
**结构化回调：**
*   `trace.add_sink(fn)` / `trace.remove_sink(fn)`: 结构化 trace 事件回调。
