...
field_on()
field_off()
active(ll=False, ignore_error=False, reqa_cmd=0x26)
# 返回 CardInfo(uid: bytes, atq: bytes, sak: int) 或 None
# ll=True: 手动底层寻卡流程（兼容非标卡）；reqa_cmd 仅 ll=True 时生效

reqa(cmd=0x26)   # REQA（7-bit 短帧），返回 TransceiveResult | None
wupa()           # WUPA，返回 TransceiveResult | None
halt()
select(cl_level, uid)                # uid: 5-byte list（含 BCC）
anticoll(cl_level, nvb, uid_prefix)  # 返回 TransceiveResult | None
```
...
# 返回 TransceiveResult | None
# TransceiveResult: .data: list[int], .rx_bits: int
```

### 断言

```python
ASSERT_EQUAL(actual, expected, msg=None)   # 注意：actual 在前
...
card_info = active(ll=True, ignore_error=True)
...
ASSERT_EQUAL(len(resp), 16, "响应长度错误")
